...
def filename_to_url(filename, cache_dir=None):
    ...
    if not os.path.exists(cache_path):
        raise OSError("file {} not found".format(cache_path))

    meta_path = cache_path + ".json"
    if not os.path.exists(meta_path):
        raise OSError("file {} not found".format(meta_path))
...
def cached_path(
    ...
    if parsed.scheme in ("http", "https", "s3"):
        ...
        raise OSError("file {} not found".format(url_or_filename))
    else:
        # Something unknown
        raise ValueError(
    ...
    def wrapper(url, *args, **kwargs):
        try:
            ...
            if int(exc.response["Error"]["Code"]) == 404:
                raise OSError("file {} not found".format(url))
            else:
                raise
