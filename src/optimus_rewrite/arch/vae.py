...
class VAE(nn.Module):
    ...
    def loss_iw(self, x0, x1, nsamples=50, ns=1):
        ...
        sf = math.log(nsamples)
        log_prob_iw = log_sum_exp(torch.cat(ll_tmp, dim=-1), dim=-1) - sf
    ...
    def nll_iw(self, x0, x1, nsamples, ns=1):
        ...
        Args:
            x0, x1:  two different tokenization results of x, where x is the
                data tensor with shape (batch, *).
            nsamples: Int
                the number of samples required to estimate marginal data
                likelihood
        Returns: Tensor1
        ...
        # TODO: note that x is forwarded twice in self.encoder.sample(x, ns)
        # and self.eval_inference_dist(x, z, param).
        ...
        sf = math.log(nsamples)
        ll_iw = log_sum_exp(torch.cat(tmp, dim=-1), dim=-1) - sf
    ...
    def eval_log_model_posterior(self, x, grid_z):
        ...
        # (batch_size, k^2, nz)
        grid_z = (
            grid_z.unsqueeze(0).expand(batch_size, *grid_z.size()).contiguous()
        )
    ...
    def calc_mi(self, test_data_batch, args):
        ...
        for batch_data in test_data_batch:
            ...
            # E_{q(z|x)}log(q(z|x)) =
            #   -0.5*nz*log(2*\pi) - 0.5*(1+logvar).sum(-1)

            neg_entropy += (
                (
                    -0.5 * nz * math.log(2 * math.pi)
                    - 0.5 * (1 + logvar).sum(-1)
                )  # noqa: E501
                .sum()
                .item()
        ...
        for i in range(len(mu_batch_list)):
            ...
            logvar = torch.cat(
                [logvar_batch_list[_] for _ in indices], dim=0
            ).cuda()
            x_batch, nz = mu.size()
            ...
            # [z_batch]
            _lg_se = log_sum_exp(log_density, dim=1)
            _lg_s = math.log(x_batch)
            log_qz += (_lg_se - _lg_s).sum(-1)
