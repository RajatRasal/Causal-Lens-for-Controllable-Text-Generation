...
(GPT/GPT-2/Transformer-XL/XLNet)
"""
...
def build_dataload_and_cache_examples(args, tokenizer, evaluate=False):
    if isinstance(tokenizer, list):
        max_gpus = max(1, args.n_gpu)
        if not evaluate:
            args.batch_size = args.per_gpu_train_batch_size * max_gpus
            file_path = args.train_data_file
        else:
            args.batch_size = args.per_gpu_eval_batch_size * max_gpus
...
def sample_sequence_conditional(
    ...
    with torch.no_grad():
        while True:
            ...
            # pdb.set_trace()
            if (
                next_token.unsqueeze(0)[0, 0].item()
                == decoder_tokenizer.encode("<EOS>")[0]
            ):
                break
...
def latent_code_from_text(text, tokenizer_encoder, model_vae, args):
    ...
    with torch.no_grad():
        ...
        pooled_hidden_fea = model_vae.encoder(
            x0, attention_mask=(x0 > 0).float()
        )[1]
...
def text_from_latent_code(latent_z, model_vae, args, tokenizer_decoder):
    ...
    text_x1 = tokenizer_decoder.decode(
...
def evaluate_latent_space(
    args, model_vae, encoder_tokenizer, decoder_tokenizer, prefix=""
):
    ...
    # Eval!
    logger.info(
        "***** Running recontruction evaluation {} *****".format(prefix)
    )
    ...
    if args.play_mode == "reconstrction":
        ...
        logger.info("Specify play_mode from [reconstrction, interpolation]")
...
def calc_rec(
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating recontruction"):
        ...
        with torch.no_grad():
            ...
            mean, logvar = model_vae.encoder.linear(pooled_hidden_fea).chunk(
                2, -1
            )
            latent_z = mean.squeeze(1)
...
def calc_interpolate(
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating interpolation"):
        ...
        with torch.no_grad():
            if sample_interval == 0 or sample_interval == args.total_sents:
                ...
                # Connect hidden feature to the latent space
                mean, _ = model_vae.encoder.linear(pooled_hidden_fea).chunk(
                    2, -1
                )
                latent_z = mean.squeeze(1)
    ...
    for step in range(num_steps + 1):
        latent_z = (
            latent_codes[0]
            + (latent_codes[1] - latent_codes[0]) * step * 1.0 / num_steps
        )
...
def interpolate(model_vae, tokenizer_encoder, tokenizer_decoder, args):
    ...
    latent_z1, coded_length1 = latent_code_from_text(
        args.sent_source, tokenizer_encoder, model_vae, args
    )
    latent_z2, coded_length2 = latent_code_from_text(
        args.sent_target, tokenizer_encoder, model_vae, args
    )
    ...
    for step in range(num_steps + 1):
        ...
        text_interpolate = text_from_latent_code(
            latent_z, model_vae, args, tokenizer_decoder
        )
...
def analogy(model_vae, tokenizer_encoder, tokenizer_decoder, args):
    ...
    latent_z3, coded_length3 = latent_code_from_text(
        args.sent_input, tokenizer_encoder, model_vae, args
    )
    ...
    text_analogy = text_from_latent_code(
        latent_z, model_vae, args, tokenizer_decoder
    )
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="An input evaluation data file to evaluate the perplexity on (a text file).",  # noqa: E501
    )
    parser.add_argument(
        "--checkpoint_dir",
        default=None,
    ...
    parser.add_argument(
        "--dataset", default="Snli", type=str, help="The dataset."
    )
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Optional pretrained config name or path if not the same as model_name_or_path",  # noqa: E501
    )
    parser.add_argument(
        "--encoder_tokenizer_name",
        default="",
        type=str,
        help="Optional pretrained tokenizer name or path if not the same as model_name_or_path",  # noqa: E501
    )
    ...
    parser.add_argument(
        ...
        default=512,
        type=int,
        help="Optional input sequence length before tokenization. The sequence will be dropped if it is longer the max_seq_length",  # noqa: E501
    )
    ...
    # Variational auto-encoder
    parser.add_argument(
        "--nz", default=32, type=int, help="Latent space dimension."
    )
    ...
    parser.add_argument(
        "--no_cuda",
        action="store_true",
        help="Avoid using CUDA when available",  # noqa: E501
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=42,
        help="random seed for initialization",  # noqa: E501
    )
    ...
    parser.add_argument(
        ...
        "The training dataset will be truncated in block of this size for training."  # noqa: E501
        "Default to the model max input length for single sentence inputs (take into account special tokens).",  # noqa: E501
    )
    parser.add_argument(
        "--do_lower_case",
        action="store_true",
    ...
    parser.add_argument(
        "--use_philly",
        action="store_true",
        help="Use Philly for computing.",  # noqa: E501
    )
    ...
    ) = MODEL_CLASSES[args.encoder_model_type]
    ...
    ) = MODEL_CLASSES[args.decoder_model_type]
    ...
    checkpoint = torch.load(
        ...
        map_location=args.device,  # noqa: E501
    )
    ...
    # Notice: resize_token_embeddings expect to receive the full size of the
    # new vocabulary, i.e. the length of the tokenizer.
    ...
    if args.interact_with_user_input:
        ...
        if args.play_mode == "interpolation":
            if len(args.sent_source) > 0 and len(args.sent_source) > 0:
                result = interpolate(
                    model_vae, tokenizer_encoder, tokenizer_decoder, args
                )
            else:
                print(
                    "Please check: specify the source and target sentences!"
                )  # noqa: E501

        if args.play_mode == "analogy":
            if (
                len(args.sent_source) > 0
                and len(args.sent_source) > 0
                and len(args.sent_input) > 0
            ):
                result = analogy(
                    model_vae, tokenizer_encoder, tokenizer_decoder, args
                )
                ...
                print(
                    "Please check: specify the source, target and input analogy sentences!"  # noqa: E501
                )

    else:
        result = evaluate_latent_space(
            args,
            model_vae,
            tokenizer_encoder,
            tokenizer_decoder,
