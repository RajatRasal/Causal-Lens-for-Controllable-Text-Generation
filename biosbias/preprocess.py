...
titles_to_ignore = {
    ...
    "magician",
    "acupuncturist",
}  # close but not enough data on these titles :-(
...
def dedup_middle(
    bios,
):  # remove triples where the middle name is a prefix of another middle name, so {Mary Lynn Doe, Mary L Doe, Mary Doe} => {Mary Lynn Doe}, but {Mary L Doe, Mary I Doe} => {Mary L Doe, Mary I Doe}
...
def dedup(people):
    ...
    ans = [
        sorted(ps, key=lambda p: (-len(p["raw"]), p["raw"], p["path"]))[0]
        for ps in by_name_title.values()
    ]
    return ans
...
def main(paths, output_filename):
    all_people = [
        p
        for path in paths
        for p in load_pkl(path)
        if p["title"] not in titles_to_ignore
    ]
    people = dedup_middle(dedup(all_people))
    print(
        f"{len(people):,}/{len(all_people):,} 'different' name+titles ({len(people)/len(all_people):.1%})"
    )
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Dedup bios by name + title and add name field to records."
    )
    parser.add_argument(
        "paths", nargs="+", help="Path of bios .pkl file(s)", metavar="PATH"
    )
    parser.add_argument(
        "-o",
        "--output",
        dest="output",
        help="write bios to OUT.pkl",
        metavar="OUT",
        default="BIOS.pkl",
    )
