...
parser.add_argument(
    "-o", "--output", dest="output", help="write bios to OUT.pkl", metavar="OUT"
)
...
parser.add_argument(
    ...
    type=int,
    default=2,
    help="number of retries per path",
)
...
with open("freq_titles_subset.json") as f:
    freq_titles = json.load(f)

lower_freq_titles = {
    t.lower(): normalized for t, normalized in freq_titles.items()
}
...
def re_escape_title(title):
    ...
    return "".join(
        f"[{c}{c.lower()}]" if c.isupper() else re.escape(c) for c in title
    )


freq_titles_regex = re.compile(
    r"\b|".join(re_escape_title(title) for title in sorted(freq_titles)) + r"\b"
)

bad_regex = re.compile(
    r"\b(?:I|you|your|me|my|mine|myself|our|ours|us|we|ourselves)\b", flags=re.I
)
...
lname_strip_regex = re.compile(
    r"^[^a-z]*(?:\b(?:[a-z]|" + acceptable_prefixes + r")\b[^a-z]*)*", re.I
)
lname_kill_regex = re.compile(
    r"^(?:about|abstract|additionally|although|and|but|by|comments|example|he|plot|review|she|source|story|summary|synopsis|the|there|today|when|while|yes)\b",
    re.I,
)
rname_regex = re.compile(
    r"(?:[\b(,\. ]+(?:\(eae\)|[a-z]|ab|abpp|aia|ao|apn|aprn|arnp|asid|asla|ba|bs|bsn|ca|cbe|ccrn|cde|cdn|cdw|ceo|cfo|cipd|clt|cnm|cnp|cpa|cpnp|crnp|csat|cso|cssd|dc|dds|djb|dmd|dnp|e\-?ryt[\- \d]*|edd|esq|faan|facs|faia|fca|fnp|fnp-bc|fnp-c|frcs|ii|iii|iv|jd|jg|jr|lac|ladc|lcpc|lcsw|ld|ldn|licsw|ll|llm|llp|lmft|lmhc|lmt|lp|lpc|ma|mba|mc|md|mfa|mft|mlc|mms|mn|mpas|mph|ms|msn|mw|ncarb|nd|np|np-c|pa-c|pa\-c|ph|phd|pla|pm|psy|psyd|ra|rcyt[\- \d]*|rd|rdn|riba|rla|rn|rn\-bc|ryt|sr)[\b\., )]*)*$",
    re.I,
)
name_regex = re.compile(
    r"^([A-Z][a-zâêîôûŵŷäëïöüẅÿàèìòùẁỳáéíóúẃý]+(?:\-[A-Z][a-zâêîôûŵŷäëïöüẅÿàèìòùẁỳáéíóúẃý]+)*)( +[A-Z](?:\.|[a-zâêîôûŵŷäëïöüẅÿàèìòùẁỳáéíóúẃý]*))?((?: van)? +(?:Mc|De|O')?[A-Z][a-zâêîôûŵŷäëïöüẅÿàèìòùẁỳáéíóúẃý]+(?:\-[A-Z][a-zâêîôûŵŷäëïöüẅÿàèìòùẁỳáéíóúẃý]+)*)$"
)
...
def extract_name(name):
    ...
    if not match:
        return None
    return tuple(
        g.strip().replace(".", "") if g else "" for g in match.groups()
    )
...
def extract_bios_from_page(
    page, URI, max_preceed=MAX_PRECEED, min_len=MIN_LENGTH
):
    if "the" not in page:
    ...
    for line_str in page.split("\n"):
        if len(line_str) >= min_len:
            ...
            if (
                not m
            ):  # try is an American architect  (1 word before title) also catches boston-based architect
                c = line_str.find(" ", b)
                if (
                    c == -1 or line_str[c - 1] == ","
                ):  # avoid 'is a performer, architect blah ...'
                    continue
            ...
            if m:
                ...
                if (
                    not g
                    or line_str[end:].startswith(",")
                    or line_str[end:].startswith(" and ")
                ):  # avoid 'is an architect and' or 'is an architect, blah'
                ...
                matches.append(
                    {
                        ...
                        "gender": g,
                        "start_pos": start_pos,
                        "title": lower_freq_titles[title.lower()],
                        "URI": URI,
                    }
                )
    return matches
...
def bios_from_wet_url(url, verbose=True):
    try:
        time0 = time.time()
        log("TRYING " + url)
        ...
        assert (
            r.status_code == 200
        ), f"*** Got status code {r.status_code} != 200"
        ...
        ans = dedup_exact(
            [
                bio
                for record in it
                for bio in extract_bios_from_page(
                    record.content_stream().read().decode()[:MAX_PAGE_LEN],
...
def process_urls(
    ...
    num_progress_reports = max(
        1, min(num_progress_reports, len(paths) // n_processes)
    )
    done = 0
...
if __name__ == "__main__":
    if not args.wetpaths.endswith("wet.paths"):
        assert re.match(
            r"^[0-9]+-[0-9]+$", args.wetpaths
        ), "Expecting wetpaths to be either xxxx-xx or to end with wet.paths"
        url = COMMON_CRAWL_URL + "crawl-data/CC-MAIN-{}/wet.paths.gz".format(
            args.wetpaths
        )
        r = requests.get(url, stream=True)
        assert r.status_code == 200
        r.raw.decode_content = (
            True  # just in case transport encoding was applied
        )
        gzip_file = gzip.GzipFile(fileobj=r.raw)
        paths = [line.decode().strip() for line in gzip_file][:10]
        print("Got {:,} urls from {}".format(len(paths), url))
        path_root = "CC-MAIN-{}-".format(args.wetpaths)
    else:
        with open(args.wetpaths) as f:
    ...
    assert output_fname.endswith(
        "bios.pkl"
    ), "Output filename must end with 'bios.pkl'"
    ...
    if len(failed_paths) < len(paths) / 10:
        for i in range(args.retries):
            ...
            more_bios, failed_paths = process_urls(
                failed_paths, n_processes=args.parallel
            )
            bios += more_bios
